...
export const GiaTriThuocTinhModal: React.FC<GiaTriThuocTinhModalProps> = ({
  ...
  useEffect(() => {
    const loadData = async () => {
      setLoading(true);
      try {
        // 1. Fetch attributes applicable for component (LINH_KIEN)
        ...
        message.error(e?.message || t('giaTriThuocTinhModal.khong_the_tai_du'));
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSave = async () => {
    try {
      ...
      if (res.code === 200) {
        message.success(t('giaTriThuocTinhModal.cap_nhat_thuoc_tinh'));
        onCancel();
      } else {
  ...
  return (
    <Modal
      title={t('giaTriThuocTinhModal.cau_hinh_thuoc_tinh_linh', { assetName: assetName })}
      open={open}
      onCancel={onCancel}
      width={600}
      footer={[
      ...
      <Spin spinning={loading}>
        {attributes.length === 0 ? (
          <div style={{ padding: 24, textAlign: 'center', color: '#8c8c8c' }}>
            Không có thuộc tính động nào được cấu hình áp dụng cho Linh kiện phần cứng.
          </div>
        ) : (
