...
export const GiaTriThuocTinhModal: React.FC<GiaTriThuocTinhModalProps> = ({
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        message.error(e?.message || 'Không thể tải dữ liệu thuộc tính linh kiện!');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSave = async () => {
    try {
      ...
      if (res.code === 200) {
        message.success('Cập nhật thuộc tính linh kiện thành công!');
        onCancel();
      } else {
  ...
  return (
    <Modal
      title={`Cấu hình thuộc tính linh kiện - ${assetName}`}
      open={open}
      onCancel={onCancel}
      width={600}
      footer={[
